...
// WICHTIG: Achte auf die geschweiften Klammern { } beim Import!
import { useStore } from '../store/useStore';

export const UI = () => {
  const { 
    slides, index, editorMode, transformMode, 
    setIndex, setEditorMode, setTransformMode, 
    updateSlide, deleteSlide, addSlide, uploadAsset, loading 
  } = useStore();
  ...
  // --- LEERE DATENBANK SCREEN ---
  if (!loading && slides.length === 0) {
    return (
      <div style={styles.emptyScreen}>
        <h1 style={{marginBottom: 20, textShadow: '0 4px 10px rgba(0,0,0,0.5)'}}>Deine Präsentation ist leer</h1>
        <button onClick={addSlide} style={styles.primaryBtn}>
  ...
  return (
    <>
      <style>{`
        ...
        .btn { transition: all 0.2s ease; transform: scale(1); }
      ...
      <div style={styles.container}>
        ...
        {editorMode && currentSlide && (
          <div className="glass-panel" style={styles.sidebar}>
            ...
            <div className="custom-scroll" style={styles.sidebarContent}>
              <div style={styles.fieldGroup}>
                <label style={styles.label}>Typ</label>
                <select 
                  value={currentSlide.type} 
                  onChange={e => updateSlide(currentSlide.id, { type: e.target.value })} 
                  style={{width:'100%'}}
                >
              ...
              {currentSlide.type === 'text' ? (
                 <>
                   ...
                   <div style={styles.fieldGroup}>
                     ...
                     <textarea 
                       ...
                       style={{width:'100%', height: 100, boxSizing: 'border-box', fontFamily: 'inherit'}} 
                     />
                   </div>
                 </>
              ) : (
                 <div style={styles.fieldGroup}>
                    ...
                    <div style={styles.uploadBox}>
                      ...
                      <p style={{fontSize:11, color:'#aaa', marginTop: 5}}>Wird sicher in Supabase gespeichert.</p>
                    </div>
                 </div>
              )}
              ...
              <div style={styles.fieldGroup}>
                ...
                <div style={{display:'flex', alignItems:'center', gap: 10}}>
                  <input 
                    ...
                    onChange={e => updateSlide(currentSlide.id, { color: e.target.value })}
                    style={{height: 40, width: 60, padding: 2, cursor: 'pointer'}}
                  />
                  <span style={{fontSize: 12, color: '#aaa'}}>{currentSlide.color}</span>
                </div>
              </div>
            </div>

            <div style={styles.sidebarFooter}>
              <button className="btn" onClick={() => deleteSlide(currentSlide.id)} style={styles.deleteBtn}>
                🗑️ Folie Löschen
              </button>
            </div>
          </div>
        ...
        <div className="glass-panel" style={styles.toolbar}>
           ...
           <button 
             ...
             style={{...styles.pillBtn, background: editorMode ? '#ff6b6b' : 'white', color: editorMode ? 'white' : 'black'}}
           >
...
const styles = {
  ...
  emptyScreen: { position:'fixed', inset:0, display:'flex', alignItems:'center', justifyContent:'center', color:'white', flexDirection:'column', background: '#1a1a1a' },
  ...
  primaryBtn: { padding: '15px 30px', fontSize: 18, cursor:'pointer', background:'#4ecdc4', border:'none', borderRadius: 30, color: '#1a1a1a', fontWeight: 'bold', boxShadow: '0 4px 15px rgba(78, 205, 196, 0.4)' },
